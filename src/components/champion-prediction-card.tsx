...
import { upsertChampionPrediction, upsertChampionOnly } from '@/lib/champion-predictions'
...
function ChampionOnlyCard({
  ...
  const isCorrect = seasonFinished && standingsChampion != null && prediction?.champion_team === standingsChampion
  const points = seasonFinished && prediction ? (isCorrect ? ROUND_ROBIN_CORRECT_POINTS : 0) : null
  ...
  async function handleSave() {
    ...
    setError('')
    try {
      await upsertChampionOnly(userId, competitionName, championTeam)
  ...
  if (!seasonFinished && !prediction && sortedTeams.length === 0) return null
  ...
  return (
    <div id="champion-prediction" className={`${className} rounded-xl p-4 bg-slate-800 border border-yellow-500/35 scroll-mt-20`}>
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-yellow-400">Predicción Campeón — {competitionName}</span>
      </div>

      {seasonFinished && (
        <div className="mb-3 p-2.5 rounded-lg bg-green-500/10 border border-green-500/30">
          <p className="text-xs font-semibold text-green-400">
            Campeón real: {standingsChampion ?? '—'}
          </p>
          {points !== null && (
            <p className="text-xs text-green-300 mt-0.5">
              {isCorrect ? '¡Acertaste!' : 'No acertaste'} <span className="text-yellow-400 font-bold">+{points} pts</span>
            </p>
          )}
          {!prediction && (
      ...
      {!seasonFinished && locked && (
        <div className="flex items-center gap-2 mb-3 px-2.5 py-2 rounded-lg bg-slate-700/40">
          <Lock className="w-3.5 h-3.5 text-amber-400 shrink-0" />
          <span className="text-xs text-slate-400">Bloqueado: ya arrancó la segunda mitad de la temporada</span>
        </div>
      )}
      ...
      {!seasonFinished && (
        <>
          <div className="mb-3">
            <label className="text-xs text-slate-500 block mb-1">Equipo campeón</label>
            <select
              disabled={locked}
              value={championTeam}
